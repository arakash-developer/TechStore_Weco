...
appServeer.get('/create', async (req, res) => {
    // res.send("Helllow");
    let createUser = await usermodel.create({
        name: "akash5",
        username: "aar2",
        email: "aar9@gmail2.com",
        describtion: "akash2 Lorem ipsum dolor sit amet, consectetur adipisicing elit. Voluptatibus quia, Nonea! Maiores et perferendis eaque, exercitationem praesentium nihil."
    })
    res.send(createUser);
})
appServeer.get('/read', async (req,res) => {
    let users = await usermodel.find();
    res.render('read',{users:users});
})
appServeer.get('/delete', async (req,res) => {
    let users = await usermodel.findOneAndDelete({name:"akash5"});
    res.send(users);
})
